...
protected:
	...
	struct Hooks
	{
		...
		static void Install()
		{
			...
			*/
            REL::Relocation<std::uintptr_t> TakeScreenshot_hook{REL::RelocationID(35882, 36853).address() + 
                                                                REL::Relocate(0x73, 0x69)};
            REL::Relocation<std::uintptr_t> WriteScreenshot_hook{REL::RelocationID(75598, 77406).address() +
                                                                 REL::Relocate(0x13B, 0x143)};
